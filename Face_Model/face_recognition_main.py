    ...
    def fetch_detections(image, embeddings, display_image_with_detections=False):
        ...
        for face in faces:
            ...
            if len(detected) > 0:
                ...
                #     name = re.findall("^[A-Za-z\s]*", detected[0])[0]
                #     cv2.putText(image, name, (x, y - 4), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)

        # if display_image_with_detections:
        #     cv2.imshow("Detected", cv2.resize(image, (700, 700)))
...
def face_recognition(image_or_video_path=None, display_image=False, face_dir="faces/"):
    ...
    while 1:
        ...
        if not ret:
            ...
            return
        detected_person, err_array = FaceDetection.fetch_detections(image, embeddings, display_image)
        ...
        # print("Detected",detected_person)
        # print("Detected Person",re.findall("^[A-Za-z\s]*", people[detected_person_min])[0])
        print("-------------------------------")
